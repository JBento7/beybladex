...
export default function EditBeybladesButton({
  tournamentId,
  deckType = "SOLO",
  currentBeybladeIds,
}: {
  ...
  useEffect(() => {
    ...
    setLoadError(false);
    setErr(null);
    setSelected(currentBeybladeIds);
    fetch("/api/beyblades")
      .then((r) => {
  ...
  async function handleSave() {
    ...
    if (res.ok) {
      ...
      setErr(data.error || "Erro ao atualizar combos");
    }
  }
  ...
  return (
    <>
      <button
        ...
        className="text-sm bg-[#252525] hover:bg-[#333] text-gray-300 font-semibold px-4 py-2 rounded-lg transition-colors"
      >
        Editar combos
      </button>
      ...
      {showModal && (
        ...
        >
          ...
          >
            <h2 className="text-lg font-bold text-white mb-1">Editar Combos do Torneio</h2>
            ...
            <div className="flex gap-3">
              ...
              >
                {loading ? "Salvando..." : `Salvar (${selected.length}/${required})`}
              </button>
            </div>
          </div>
        </div>
